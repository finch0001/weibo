...
public class WeiboException extends Exception {
	...
	public WeiboException(Context context) {
		this.context = context;
	}

	public WeiboException(Context context, String detailMessage) {
		this.context = context;
		error = detailMessage;
	}
	...
	public String getError() {
		...
		if (!TextUtils.isEmpty(error)) {
			...
			int i = context.getResources().getIdentifier(name, "string",
					context.getPackageName());
			try {
				result = context.getString(i);
			} catch (Resources.NotFoundException e) {
				if (!TextUtils.isEmpty(oriError)) {
					result = oriError;
				} else {
					result = context
							.getString(R.string.unknown_error_error_code)
							+ error_code;
				}
			}
		}
